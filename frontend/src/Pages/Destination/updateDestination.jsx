...
import { getDestinationById, updateDestinationById } from "../../Api/services/destinationService";
...
export default function UpdateDestination() {
  ...
  // Manage images as URL strings (instead of files)
  const [image1, setImage1] = useState("");
  const [image2, setImage2] = useState("");
  const [googlemap, setGooglemap] = useState(data?.location || "");
  const [openModal, setOpenModal] = useState(false);
  const [loading, setLoading] = useState(false);
  const [progress, setProgress] = useState(0);
  const [title, setTitle] = useState(data?.title || "");
  const [maindescription, setMaindescription] = useState(data?.maindescription || "");
  ...
  // Validation schema: images are now required to be valid URLs.
